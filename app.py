...
def request_onload(api_key,api_version,api_province):
    url = f"https://api.getbuildify.com/{api_version}/{api_province}/search_listings?page=0&perPage=0&facets=type%2Cneighbourhood%2CcityOrDistrict&filterQuery=minBeds%20%3E%3D%201%20AND%20minBaths%20%3E%3D%201%20"
    ...
    req_json["facets"]["startPrice"]=min_max_price(api_key,api_version,api_province)
    req_text = json.dumps(req_json)
    return req_text
    ...
    while True:
        ...
        for elem in result:
            if isinstance(elem["startPrice"], int) and not isinstance(elem["startPrice"], bool):
                if flag_elem==True:
                    flag_elem=False
                    min_max_array=[elem["startPrice"],elem["startPrice"]]
                min_max_array=[min(min_max_array[0],elem["startPrice"]),max(min_max_array[1],elem["startPrice"])]
        count+=1
        if req_json["total"]<=count*1000:
            return {min_max_array[0]: 0, min_max_array[1]:0}
